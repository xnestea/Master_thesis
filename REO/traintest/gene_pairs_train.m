...
load ("Train_data_REO.mat")

c1_res = sig_stable_gene_pair(rownames,T1.data,0.00000001);
T_rest = [T2.data T3.data];
c_23_res = sig_stable_gene_pair(rownames,T_rest, 0.00000001);
c1_res = c1_res{1,1};
...
c2_res = sig_stable_gene_pair(rownames,T2.data,0.00000001);
T_rest = [T1.data T3.data]; 
c_13_res = sig_stable_gene_pair(rownames, T_rest,0.00000001);
c2_res = c2_res{1,1};
...
c3_res = sig_stable_gene_pair(rownames,T3.data,0.00000001);
T_rest = [T1.data T2.data];
c_12_res = sig_stable_gene_pair(rownames, T_rest,0.00000001);
c3_res = c3_res{1,1};
